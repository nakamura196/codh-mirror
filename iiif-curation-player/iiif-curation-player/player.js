...
var IIIFCurationPlayer = function(config) {
    ...
    var VERSION  = '2.0.1+20241220';
    if (window.console) {
        console.log(APP_NAME + ' v' + VERSION); // eslint-disable-line no-console
    }
    var APP_URL = 'http://codh.rois.ac.jp/software/iiif-curation-player/';
    ...
    var enableAnnotationViewMode = false; //ICPlayerでは常に無効とする

    var err;
    var inModalTransitions = 0;

    var enableCurationEdit = false;
    var storage;
    try {
    ...
    var defaultConfig = {
        ...
        descriptionDialog: { //整数＋単位で指定（ピクセル単位であれば'px'、マップの大きさに対する相対指定であれば'%'）
            defaultWidth: '50%',
            defaultHeight: '95px'
        }
    };
    ...
    function configure(config, defaultConfig) {
        ...
        if ($.isPlainObject(config)) {
            ...
            if ($.isPlainObject(config.descriptionDialog)) {
                if ($.type(config.descriptionDialog.defaultWidth) === 'string') {
                    conf_.descriptionDialog.defaultWidth = config.descriptionDialog.defaultWidth;
                }
                if ($.type(config.descriptionDialog.defaultHeight) === 'string') {
                    conf_.descriptionDialog.defaultHeight = config.descriptionDialog.defaultHeight;
                }
            }
        }
    ...
    function isBorderMarkerEditingEnabled() {
        //ICPlayerでは、枠マーカーの領域編集機能は提供しない（有効にした場合の動作は未検証）
        return false; //(conf.curation.enableRectangleMarkerEdit && curationInfo.curation && curationInfo.curation.viewingHint === 'annotation');
    }
    ...
    function processCurationUrl(curationUrl, options) {
        function processCurationCore(curationUrl, curation_) {
            if (isValidCurationFalseTrue(curation_)) {
                ...
                if (enableAnnotationViewMode && (getMode() === 'annotation' || curation_.viewingHint === 'annotation')) {
                    //アノテーションビューモード
                    setMode('annotation');
    ...
    function refreshPage() {
        ...
        function showDescriptionDialog(description) {
            ...
            var dialogWidth = unitToPixel(conf.descriptionDialog.defaultWidth, mapWidth - 20, mapWidth / 2);
            var dialogMinWidth = 100;
            if (windowWidth < 768) {
                var zoomControlWidth = (L.Browser.touch) ? 34 : 26;
                dialogWidth = mapWidth - zoomControlWidth - 30;
            }
            if (dialogWidth < dialogMinWidth) {
                dialogWidth = dialogMinWidth;
            }
            var dialogHeight = unitToPixel(conf.descriptionDialog.defaultHeight, mapHeight - 20, 95);
            var dialogMinHeight = 56;
            if (windowWidth < 768) {
                dialogHeight = (L.Browser.touch) ? 106 : 88;
            }
        ...
        // map.addControl(drawControlDrawOnly); //ICPlayerでは領域選択機能は無効にする
    ...
    function setupNavigations() {
        ...
        //ICPlayerは、ICViewer等と異なり、curationのラベルがあればmanifestのラベルよりも優先表示する。
        ...
        if ($curationDescriptionButton) {
            var $curationDescriptionButton_ = $('<small>').append($curationDescriptionButton);
            $('#book_title').append($curationDescriptionButton_);
        }
        ...
        //ビューモード切替
        if (enableAnnotationViewMode && getBrowsingCurationUrl() && curation.viewingHint !== 'annotation') {
